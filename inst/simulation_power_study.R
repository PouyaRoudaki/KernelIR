...
#n <- 20
#n_sim <- 10
#B <- 30                 # number of permutations per dataset

alpha <- 0.05

lambda_vals <- seq(0, 1, 0.1)
alternatives <- c("linear", "step", "W", "Sin", "circular",
                  "heteroskedastic",
                  "spiral",
                  "x_shape", "two_moons","two_sines",
                  "lissajous_curve",
...
stats_list <- list(
  ...
  mic       = function(x, y) minerva::mine(x = x, y = y)$MIC,

  dcor      = function(x, y) energy::dcor(x, y),

  # FIX: extract the scalar dHSIC value
  hsic      = function(x, y) dHSIC::dhsic(x, y)$dHSIC,  # or $dHSIC[[1]]

  hhg       = function(x, y) max(OurHHG::hhg.univariate.ind.stat(x, y)$statistic),

  codec     = function(x, y) FOCI::codec(Y = y, Z = x),
  kpcrkhs   = function(x, y) KPC::KPCRKHS(Y = y, Z = x),
  kmagraph_1nn  = function(x, y) KPC::KMAc(Y = y, X = x, Knn = 1),
  kmagraph_5nn  = function(x, y) KPC::KMAc(Y = y, X = x, Knn = 5),

  kir_rkhs      = function(x, y) KernelIR::kir_rkhs(Y = y, X = x),
  kir_graph_1nn = function(x, y) KernelIR::kir_graph(Y = y, X = x, Knn = 1),
  kir_graph_5nn = function(x, y) KernelIR::kir_graph(Y = y, X = x, Knn = 5),

  irdc      = function(x, y) FORD::irdc(Y = y, X = x),
  irdcs     = function(x, y) FORD::irdc_simple(Y = y, X = x)
)
...
compute_power <- function(type, lambda) {
  results <- foreach(i = 1:n_sim, .combine = rbind,
                     .packages = c("XICOR","minerva","energy","dHSIC","OurHHG","FORD","FOCI","KPC","KernelIR"),
...
for (type in alternatives) {
  ...
  saveRDS(powers_df, file = paste0("results/power_", type, ".rds"))
